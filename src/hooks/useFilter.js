...
const useFilter = (data) => {
  ...
  const serviceData = useMemo(() => {
    ...
    if (brandType) {
      services = services.filter((search) =>
        search.type.toLowerCase().includes(brandType.toLowerCase())
      );
    }
    ...
    if (storeType) {
      services = services.filter((search) =>
        search.type.toLowerCase().includes(storeType.toLowerCase())
      );
    }
  ...
  const handleSubmitBrand = (e) => {
    e.preventDefault();
    setBrandType(brandRef.current.value);
  };

  const handleSubmitStore = (e) => {
    e.preventDefault();
    setStoreType(storeRef.current.value);
  };
